...
impl Aig {
    ...
    pub fn coi_refine_preserve(&self, preserved_vars:&GHashSet<Var>) -> (Aig, GHashMap<Var, Var>) {
        ...
        //     .map(|e| e.node_id())
        //     .collect();
        trace!("Refining AIG with preserved variables: {:?}", preserved_vars);
        let refine_root: Vec<usize> = self
            .constraints
            .iter()
            .chain(self.outputs.iter())
            ...
            .map(|e| e.node_id())
            .chain(
                preserved_vars.iter().map(|v| v.0 as usize) // Add node IDs from preserved_vars
            )
            .collect();
